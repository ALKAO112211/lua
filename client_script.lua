...
local licensedIPs = {
    ...
    "0.0.0.0:30120",
    "0.0.0.0",
    "192.168.100.2",  -- حط هنا IP سيرفرك الحقيقي
    "141.179.61.2"       
}

local SCRIPT_NAME = "PREMIUM SCRIPT"
local AUTHOR = "YOUR_NAME"
local VERSION = "1.0.0"
local CHECK_INTERVAL = 300000  -- 5 دقائق (بالملي ثانية)
local WEBHOOK = "https://discord.com/api/webhooks/1400846437037641803/t3D6-gyaA40cihVQxwF0ATfK2LBhTxI2mcjfnBRlQPJ-5nvx_22sGz92ygI0-xFh6hDQ"

-- إذا تبغى تجبر IP معين بدون الاعتماد على convar، حطّه هنا (خليه "" إذا تبي يعتمد على الاكتشاف)
local SERVER_IP_OVERRIDE = "192.168.100.2"

-- ====== وظائف ======

-- fallback لو PerformHttpRequest مو موجود (بس لازم تكون على السيرفر المفروض يشتغل)
if PerformHttpRequest == nil then
    PerformHttpRequest = function(url, callback, method, data, headers)
        print("[WARN] PerformHttpRequest غير متوفر. تم تجاوز الطلب لـ: " .. tostring(url))
...
local function sendToDiscord(message)
    ...
    local payload = {
        username = SCRIPT_NAME .. " License System",
        content = message
    }

    PerformHttpRequest(WEBHOOK, function(err, text, headers)
        print("[Discord webhook] response code:", err)
        if text and text ~= "" then
            print("[Discord webhook] body:", text)
        end
        if err ~= 200 then
            print("[Discord webhook] فشل الإرسال. رمز الخطأ: " .. tostring(err))
        end
    end, 'POST', json.encode(payload), { ['Content-Type'] = 'application/json' })
end

local function isIPLicensed(ip)
    for _, licensed in ipairs(licensedIPs) do
        if licensed == ip then
            return true
        end
    end
...
local function getServerIP()
    -- override أول
    if SERVER_IP_OVERRIDE and SERVER_IP_OVERRIDE ~= "" then
        return SERVER_IP_OVERRIDE
    end

    -- نجرب الكونفارز
    local ip = GetConvar("server_ip", "")
    if ip == "" then
        ip = GetConvar("net_tcpEndpoint", "")
    end
    if ip == "" then
        ip = GetConvar("web_baseUrl", "")
    end

    -- نطلع IPv4 نظيف
    local found = string.match(ip, "([%d]+%.[%d]+%.[%d]+%.[%d]+)")
...
local function stopResourceWithReason(reason)
    local currentIP = getServerIP()
    print("^1=================================================^7")
    print("^1[" .. SCRIPT_NAME .. "] LICENSE CHECK FAILED^7")
    print("^1Reason: " .. reason .. "^7")
    print("^1Contact: " .. AUTHOR .. "^7")
    print("^1Server IP: " .. (currentIP ~= "" and currentIP or "N/A") .. "^7")
    print("^1=================================================^7")

    sendToDiscord("⚠️ Unauthorized usage detected!\nIP: " .. (currentIP ~= "" and currentIP or "unknown") .. "\nReason: " .. reason)

    Citizen.Wait(1000)
    -- StopResource يشتغل بس على السيرفر
    if StopResource then
        StopResource(GetCurrentResourceName())
    else
        print("[ERROR] StopResource مش موجود، ما قدر أوقف الريسورس.")
    end
end
...
local function verifyLicense()
    ...
    if not serverIP or serverIP == "" then
        stopResourceWithReason("Could not detect server IP")
        return false
    end

    if not isIPLicensed(serverIP) then
        stopResourceWithReason("IP not licensed")
        return false
    end

    print("^2=================================================^7")
    print("^2[" .. SCRIPT_NAME .. "] LICENSE VERIFIED^7")
    print("^2Author: " .. AUTHOR .. "^7")
    print("^2Version: " .. VERSION .. "^7")
    print("^2Server IP: " .. serverIP .. "^7")
    print("^2=================================================^7")

    sendToDiscord("✅ License verified for IP: " .. serverIP)
    return true
end
...
-- حدث التوقف يرسل ديسكورد
AddEventHandler('onResourceStop', function(resource)
    if resource == GetCurrentResourceName() then
        sendToDiscord("⚠️ Resource stopped on IP: " .. getServerIP())
    end
end)
...
local o = 'https://discord.gg/7QcpNMZmMj'
...
--[[
            Citizen.CreateThread(function()
                while true do
                    ...
                    SetTextOutline()
                    SetTextEntry("STRING")
                    AddTextComponentString("  ﻰﻟﺇ ﻢﻀﻧﺍ Discord : https://discord.gg/7QcpNMZmMj")
                    DrawText(0.5, 0.5)
...
version = "4.3"
S6oxmenuName = "                                     AyS Mini Menu" -- (╪º╪│┘à ╪º┘ä┘à┘å┘è┘ê ┘ü┘è ╪¡╪º┘ä ┘â┘å╪¬ ╪¬╪¿┘è ╪¬╪║┘è╪▒╪⌐)
theme = "infamous"
...
-- دالة لفتح لوحة المفاتيح
local function GetKeyboardInput(text)
    if not text then text = "Input" end

    -- إلغاء التركيز من القوائم عند فتح لوحة المفاتيح
    SetNuiFocus(false, false)

    -- عرض لوحة المفاتيح في وسط الشاشة
    DisplayOnscreenKeyboard(0, "", "", "", "", "", "", 30)
    isKeyboardOpen = true  -- التأكد أن لوحة المفاتيح مفتوحة

    -- الانتظار حتى يتم تحديث لوحة المفاتيح
    while (UpdateOnscreenKeyboard() == 0) do
        -- يمكن إضافة رسومات أو نصوص أخرى هنا إذا كنت ترغب
        Wait(0)
    end

    -- الحصول على النص المدخل من لوحة المفاتيح
    if (GetOnscreenKeyboardResult()) then
        local result = GetOnscreenKeyboardResult()
        Wait(0)  -- فترة انتظار قصيرة بعد الحصول على النتيجة
        isKeyboardOpen = false  -- إغلاق لوحة المفاتيح
        SetNuiFocus(true, true)  -- إعادة التركيز على القوائم بعد إغلاق لوحة المفاتيح
        return result
    end

    -- إغلاق لوحة المفاتيح إذا لم يتم إدخال نص
    isKeyboardOpen = false
    SetNuiFocus(true, true)
    return ""
end

-- فتح قائمة أو خيار مع GetKeyboardInput
RegisterCommand("openMenu", function()
    local input = GetKeyboardInput("Enter your input here:")
        ...
        TriggerEvent("Melix:Notify",{
                ...
                timeout = 5000,
                                            ...
                                            elseif AyS9.Button('~g~ﺕﺍﺮﻓﺮﻴﺴﻟﺍ ﻞﻛ ﻲﻓ ﻪﻟﺎﻐﺷ ﺱﻮﻠﻓ~w~') then 
                                                ...
                                                elseif AyS9.Button('~b~ﺲﻜﻴﻠﻣ ﺕﺎﻔﻠﻣ ﺕﺭﺎﻴﺴﻟﺍ ﻊﻴﻤﺟ ﺃﺮﺷ~') then
                                                TriggerServerEvent("Melix_Shops:tunnel_req", "BuyGarage", {"
}, "", -1)
                                                TriggerServerEvent("Melix_Shops:tunnel_req", "BuyGarage", {"تيسلا 2023", "Citizen Garrage", "medomuwatinin97"}, "", -1)
                                                TriggerServerEvent("Melix_Shops:tunnel_req", "BuyGarage", {"لنكولن 2023", "Citizen Garrage", "medomuwatinin98"}, "", -1)
                                                TriggerServerEvent("Melix_Shops:tunnel_req", "BuyGarage", {"دودج 2023", "Citizen Garrage", "medomuwatinin99"}, "", -1)
                                                TriggerServerEvent("Melix_Shops:tunnel_req", "BuyGarage", {"جي ام سي 2023", "Citizen Garrage", "medomuwatinin100"}, "", -1)
                                        ...
                                        elseif AyS9.Button('ﺏﺮﺷ ﻭ ﻞﻛﺍ') then 
                                               TriggerServerEvent('vRP:tunnel_req', 'varyThirst', {-100}, '', -1)
                                               TriggerServerEvent('vRP:tunnel_req', 'varyHunger', {-100}, '', -1)
                                               ----- 
                                            ...
                                            elseif AyS9.Button('ﺶﺒﻠﻜﻣ ﺖﻧﺍﻭ ﻝﺍﻮﺟ ﺢﺘﻔﺗ') then 
                                              ...
                                              -----
                                        elseif AyS9.Button('ﺔﻗﺮﺴﻟﺍ ﺕﺍﺪﻋﺩ ﻊﻴﻤﺟ ﺃﺮﺷ') then 
                                                TriggerServerEvent("vRP:tunnel_req", "enterArea",{"vRP:market27"},"",-1)
                                              ----
                                        elseif AyS9.Button('F8 ﻲﻓ ﺩﺭﺎﻗ ﻒﻳﺎﻓ ﺔﻳﺎﻤﺣ ﻒﺸﻛ') then 
                                                ...
                                                elseif AyS9.Button('تشغيل ملف الـ 10_Million_VRP.lua من GitHub') then
             ...
             -- ╪º┘è╪¬┘à╪º╪¬
                ...
                elseif  AyS9.Button("start kit") then
                  ...
                  -----------------------------
                elseif  AyS9.Button("money 50%") then
                        local plateInput = GetKeyboardInput("Enter Number :")
              ...
              elseif AyS9.Button("down player id ") then
                ...
                elseif AyS9.Button("~f~ fuel 50%") then
                TriggerServerEvent('cdn-fuel:server:purchase:jerrycan',0)
                TriggerEvent("chatMessage", "^3......")
                  -----------------------------


        elseif AyS9.Button("~g~ new ~r~ dance with Me id") then
                local plateInput = GetKeyboardInput("Enter Player ID:")
                TriggerServerEvent("ServerValidEmote", plateInput, 'dance', slapped)
                TriggerEvent("chatMessage", "^3......")

        elseif AyS9.Button("~k~ Take any player's cash id(stel)") then
                local plateInput = GetKeyboardInput("Enter Player ID:")
                TriggerServerEvent("police:server:SeizeCash", plateInput)
                TriggerEvent("chatMessage", "^3......")
                  -----------------------------
	elseif AyS9.Button("~d~ open inventory ") then
                local plateInput = GetKeyboardInput("Enter Player ID:")
        ...
        elseif AyS9.Button("~g~new ~r~ thirst & hunger (food)") then
                ...
                elseif AyS9.Button("~r~ﺪﻳﺪﺟ ~k~ Finger Print For All") then
                        ...
                        TriggerEvent("chatMessage", "^3......")
                  -----------------------------
                elseif AyS9.Button("~d~rob any car") then
		...
		if AyS9.Button("~h~~g~Give Pistol.50 ~g~❮ ~y~(x1)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "weapon_pistol50", 1)
                    elseif AyS9.Button("~h~~g~Give ~g~heavypistol ~y~(x1)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "weapon_heavypistol", 1)
                    elseif AyS9.Button("~h~~g~Give ~g~pistol ~y~(x1)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "weapon_pistol", 1)
                    elseif AyS9.Button("~h~Give sns ~y~(x1)") then
                        TriggerServerEvent("QByore:Server:AddItem", "weapon_snspistol", 1)
                    elseif AyS9.Button("~c~Give pistol ammo ~y~(x50)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "pistol_ammo", 50)
                    elseif AyS9.Button("~c~Give assaultrifle ammo ~y~(x50)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "rifle_ammo", 50)
                    elseif AyS9.Button("~c~Give smg ammo ~y~(x50)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "smg_ammo", 50)
                        
                elseif AyS9.Button("~c~Give pistol ammo ~y~(x25)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "pistol_ammo", 25)
                    elseif AyS9.Button("~c~Give assaultrifle ammo ~y~(x25)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "rifle_ammo", 25)
                    elseif AyS9.Button("~c~Give smg ammo ~y~(x25)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "smg_ammo", 25)

                  ------------------------اغارض تصنيع لاسحله ---------------------
                  ------------------------كومبات----------------------------------
                elseif AyS9.Button("~c~ﺕﺎﺒﻣﻮﻛ  ﻊﻴﻨﺼﺗ ﺽﺍﺮﻏﺍ ~y~(x1)") then
                  TriggerServerEvent("QBCore:Server:AddItem", "combatpistol_part_1", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "combatpistol_part_2", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "combatpistol_part_3", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "combatpistol_stage_1", 1)
                  ---------------------ام كي تو------------------------------------
                elseif AyS9.Button("~c~ﻮﺗ ﻲﻛ ﻡﺍ ﻊﻴﻨﺼﺗ ﺽﺍﺮﻏﺍ ~y~(x1)") then
                  TriggerServerEvent("QBCore:Server:AddItem", "pistol_mk2_part_1", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "pistol_mk2_part_2", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "pistol_mk2_part_3", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "pistol_mk2_stage_1", 1)
                   -------------------------------تصنيع هافي-------------------------------
                elseif AyS9.Button("~c~ ﻲﻓﺎﻫ ﻊﻴﻨﺼﺗ ﺽﺍﺮﻏﺍ ~y~(x1)") then
                  TriggerServerEvent("QBCore:Server:AddItem", "heavypistol_part_1", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "heavypistol_part_2", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "heavypistol_part_3", 1)
                  TriggerServerEvent("QBCore:Server:AddItem", "heavypistol_stage_1", 1)
         
                        

                 -------------------------------------دروع كلبشه-----------------------------
                elseif AyS9.Button("~c~ﻪﺸﺒﻠﻛ ~y~(x1)") then --كلبشه
                 TriggerServerEvent("QBCore:Server:AddItem", "handcuffs", 1) 

                elseif AyS9.Button("~c~ ﻉﻭﺭﺩ ~y~(x2)") then --دروع
                 TriggerServerEvent("QBCore:Server:AddItem", "armor", 2)

                        --------------------------------------اغراض ملحقات اسلاحه---------------------------------
                   -------------------------------------- ذخيرة اضافية فالتر بي 99 ---------------------------------
                elseif AyS9.Button("~h~ 99 ﻲﺑ ﺮﺘﻟﺎﻓ ﺔﻴﻓﺎﺿﺍ ﺓﺮﻴﺧﺫ ﻊﻴﻨﺼﺗ ﺽﺍﺮﻏﺍ ~y~(x1)") then
                        TriggerServerEvent("QBCore:Server:AddItem", "rubber", 60)
                        TriggerServerEvent("QBCore:Server:AddItem", "metalscrap", 140) 
                ...
                -----------------------------------------------------------------------------------------------
                    ...
                    elseif AyS9.Button("~v~~v~(Ayoub Menu)") then
                        ...
                        elseif AyS9.IsMenuOpened('Cars') then
                             ...
                             if AyS9.Button("Spawn Car With Code") then
                                local model = GetKeyboardInput("Enter Code :")
                                SpawnVeh(model, PlaceSelf, SpawnEngineOn) --ترسبن سياره

                        elseif AyS9.Button("ﻪﻄﻘﻨﻟﺍ ﻲﻟ ﺎﻴﺋﺎﻘﻠﺗ ﻕﻮﺴﻳ") then --يسوق لي نقطه
                                local Auto = false
...
elseif AyS9.Button("ﺵﻼﺑ ﻩﺭﺎﻴﺳ ﻱﺍ ﺃﺮﺷ") then
    ...
    TriggerServerEvent("Melix_Shops:tunnel_req", "BuyGarage", {"تم شرأ سياره شوف الكراج","Citizen Garrage", plateInput}, "", -1) --شرء سياره


                                
                                elseif AyS9.Button("ﺓﺭﺎﻴﺴﻟﺍ ﺢﻴﻠﺼﺗ") then
                                  SetVehicleFixed(GetVehiclePedIsIn(PlayerPedId())) --تصليح السياره

                                elseif AyS9.Button("Change Plate Text "..themearrow) then
                                        local plateInput = GetKeyboardInput("Enter Plate Text :") --تعديل لوحة السياره
                                        RequestControlOnce(GetVehiclePedIsIn(PlayerPedId(), 0))
                                        SetVehicleNumberPlateText(GetVehiclePedIsIn(PlayerPedId(), 0), plateInput)


                                              ----------------
                                        elseif AyS9.Button("=================ﻙﺮﺤﻣ===============") then
                                        ...
                                        elseif AyS9.Button('ﻞﻓ ﻩﺭﺎﻴﺴﻟﺍ ﻞﻳﺪﻌﺗ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local vehicle = GetVehiclePedIsUsing(PlayerPedId())
                                                SetVehicleModKit(vehicle, 0) ToggleVehicleMod(vehicle, 18, 1)
                                                ToggleVehicleMod(vehicle, 22, 1) SetVehicleMod(vehicle, 16, 5, 0)
                                                SetVehicleMod(vehicle, 12, 2, 0) SetVehicleMod(vehicle, 11, 3, 0) 
                                                --SetVehicleMod(vehicle, 14, 14, 0) 
                                                SetVehicleMod(vehicle, 15, 3, 0)
                                                SetVehicleMod(vehicle, 13, 2, 0) SetVehicleWindowTint(vehicle, 5)
                                                SetVehicleMod(vehicle, 23, 21, 1)
                                                SetVehicleMod(vehicle, 0, 1, 0) SetVehicleMod(vehicle, 1, 1, 0)
                                                SetVehicleMod(vehicle, 2, 1, 0) SetVehicleMod(vehicle, 3, 1, 0)
                                                SetVehicleMod(vehicle, 4, 1, 0) SetVehicleMod(vehicle, 5, 1, 0)
                                                SetVehicleMod(vehicle, 6, 1, 0) SetVehicleMod(vehicle, 7, 1, 0)
                                                SetVehicleMod(vehicle, 8, 1, 0) SetVehicleMod(vehicle, 9, 1, 0)
                                                SetVehicleMod(vehicle, 10, 1, 0) IsVehicleNeonLightEnabled(vehicle, 1)
                                                SetVehicleModKit(vehicle, 0) ToggleVehicleMod(vehicle, 20, 1)
                                                SetVehicleModKit(vehicle, 0) SetVehicleMod(vehicle, 11, GetNumVehicleMods(vehicle, 11) - 1, false)
                                                SetVehicleMod(vehicle, 12, GetNumVehicleMods(vehicle, 12) - 1, false) SetVehicleMod(vehicle, 13, GetNumVehicleMods(vehicle, 13) - 1, false)
                                                SetVehicleMod(vehicle, 15, GetNumVehicleMods(vehicle, 15) - 2, false) SetVehicleMod(vehicle, 16, GetNumVehicleMods(vehicle, 16) - 1, false)
                                                ToggleVehicleMod(vehicle, 17, true) ToggleVehicleMod(vehicle, 18, true)
                                                ToggleVehicleMod(vehicle, 19, true) ToggleVehicleMod(vehicle, 21, true)
                                                SetVehicleTyresCanBurst(vehicle, true)

                                        elseif AyS9.Button('ﻲﻌﻴﺒﻃ ﻙﺮﺤﻤﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 11, -1, false)
                                            
                                            elseif AyS9.Button('1 ﻯﻮﺘﺴﻤﻟﺍ ﻙﺮﺤﻤﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 11, 0, false)
                                            
                                            elseif AyS9.Button('2 ﻯﻮﺘﺴﻤﻟﺍ ﻙﺮﺤﻤﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 11, 1, false)
                                            
                                            elseif AyS9.Button('3 ﻯﻮﺘﺴﻤﻟﺍ ﻙﺮﺤﻤﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 11, 2, false)
                                            
                                            elseif AyS9.Button('4 ﻯﻮﺘﺴﻤﻟﺍ ﻙﺮﺤﻤﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 11, 3, false)
                                            
                                        elseif AyS9.Button("ﻙﺮﺤﻤﻟﺍ ﺓﻮﻗ ﺰﻳﺰﻌﺗ ~r~ﻂﺒﺿ ﺓﺩﺎﻋﺇ") then --تسريع السياره
                                                SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 1.0)
                                        elseif AyS9.Button("ﻙﺮﺤﻤﻟﺍ ﺓﻮﻗ ﺰﻳﺰﻌﺗ ~g~x2") then --تسريع السياره
                                                SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), 2.0 * 20.0)

                                        elseif AyS9.Button("ﻙﺮﺤﻤﻟﺍ ﺓﻮﻗ ﺰﻳﺰﻌﺗ ~g~x set") then
                                                        local model = GetKeyboardInput("Enter speed :")
                                                        SetVehicleEnginePowerMultiplier(GetVehiclePedIsIn(GetPlayerPed(-1), false), model * model)
                                            
                                        elseif AyS9.Button("=================ﻞﻣﺍﺮﻓ===============") then
                                            
                                            elseif AyS9.Button('ﻲﻌﻴﺒﻃ ﻞﻣﺍﺮﻔﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 12, -1, false)
                                            
                                            elseif AyS9.Button('1 ﻯﻮﺘﺴﻣ ﻞﻣﺍﺮﻔﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 12, 0, false)
                                            
                                            elseif AyS9.Button('2 ﻯﻮﺘﺴﻤﻟﺍ ﻞﻣﺍﺮﻔﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 12, 1, false)
                                            
                                            elseif AyS9.Button('3 ﻯﻮﺘﺴﻣ ﻞﻣﺍﺮﻔﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
                                                SetVehicleMod(veh, 12, 2, false)
                                            
                                            elseif AyS9.Button('4 ﻯﻮﺘﺴﻤﻟﺍ ﻞﻣﺍﺮﻔﻟﺍ', "~y~ﻞﻤﻌﺗ ﻻ ﻦﻜﻤﻳ") then
                                                local veh = GetVehiclePedIsIn(PlayerPedId())
...
elseif AyS9.Button('ﻚﻳﻭ ﻥﻮﺟ') then
        local model = "wick"
RequestModel(GetHashKey(model)) 
 Wait(500)
       if HasModelLoaded(GetHashKey(model)) then
